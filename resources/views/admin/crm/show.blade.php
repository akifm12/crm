    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('crm.edit', $crm->id) }}"
               class="flex items-center gap-1.5 px-4 py-1.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                Edit
            </a>
            <form method="POST" action="{{ route('crm.destroy', $crm->id) }}"
                  onsubmit="return confirm('Delete {{ $crm->company_name }}? This cannot be undone.')">
                @csrf @method('DELETE')
                <button type="submit"
                        class="flex items-center gap-1.5 px-4 py-1.5 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-600 transition">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-2 flex flex-wrap gap-3 items-center">
                                ...
                                <form method="POST" action="{{ route('crm.trainings.delete', $tr->id) }}"
                                    ...
                                    @csrf @method('DELETE')
                                    <button type="submit" class="flex items-center gap-1 text-xs text-red-600 bg-red-50 border border-red-200 hover:bg-red-100 font-medium px-2 py-1 rounded-lg transition">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        Delete
                                    </button>
                                </form>
