...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-xs text-gray-400 mt-2">Last screened: {{ $crm->screening_date->format('d M Y, H:i') }}</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-xs text-gray-400 mb-3">
                <strong>{{ $crm->company_name }}</strong>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div class="space-y-2">
                ...
                @php $shResult = $screenResult['shareholders'][$sh->id] ?? null; @endphp
                <div class="flex items-center justify-between gap-2 p-2 bg-gray-50 rounded-lg">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        <form method="POST" action="{{ route('screening.shareholder', $sh->id) }}">
                            @csrf
                            <button type="submit" class="text-xs text-blue-600 hover:underline font-medium">Screen</button>
                        </form>
                    </div>
                </div>
                @endforeach
    ...
    <div class="lg:col-span-2">
        ...
        {{-- Company result --}}
        <div class="bg-white rounded-xl border {{ ($screenResult['status'] ?? '') === 'match' ? 'border-red-200' : 'border-gray-200' }} mb-4">
            <div class="px-5 py-4 border-b {{ ($screenResult['status'] ?? '') === 'match' ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100' }} rounded-t-xl flex items-center justify-between">
                <h3 class="text-sm font-semibold {{ ($screenResult['status'] ?? '') === 'match' ? 'text-red-700' : 'text-gray-700' }}">
                    {{ $crm->company_name }} — screening result
                </h3>
            ...
            <div class="divide-y divide-gray-100">
                ...
                <div class="px-5 py-3 flex items-center justify-between">
                    ...
                    <span class="text-xs px-2 py-0.5 rounded-full {{ ($shResult['status'] ?? '') === 'match' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                        {{ ($shResult['status'] ?? '') === 'match' ? ($shResult['total_hits'] ?? 0).' hit(s)' : 'Clear' }}
                    </span>
                </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-xl border border-dashed border-gray-300 p-16 text-center">
            ...
            <p class="text-xs text-gray-400 mt-1">Click "Screen company now" to run a sanctions check.</p>
        </div>
        @endif
    </div>
</div>
