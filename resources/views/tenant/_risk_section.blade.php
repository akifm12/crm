    ...
    <div class="divide-y divide-gray-50">
        ...
        <div class="px-5 py-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2"
                 x-data="{ val: {{ $saved($key, $default) }} }"
                 x-init="$watch('val', v => $dispatch('factor-change', { key: '{{ $key }}', value: v }))">
                ...
                <label class="relative flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-all"
                    ...
                    <input type="radio" name="_rf_{{ $key }}" value="{{ $score }}"
                           x-model.number="val"
                           @change="$root.setFactor('{{ $key }}', {{ $score }})"
                           {{ $saved($key, $default) == $score ? 'checked' : '' }}
                           class="sr-only">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold mb-0.5"
                           :class="val === {{ $score }} ? 'text-white' : '{{ $score == 1 ? 'text-green-700' : ($score == 2 ? 'text-amber-700' : 'text-red-700') }}'">
                            {{ $score == 1 ? 'Low' : ($score == 2 ? 'Medium' : 'High') }}
                        </p>
                        <p class="text-xs leading-relaxed"
                           :class="val === {{ $score }} ? 'text-white/80' : 'text-gray-500'">
                            {{ $desc }}
                        </p>
                    </div>
                </label>
