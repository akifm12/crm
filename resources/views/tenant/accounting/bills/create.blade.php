...
@section('title', 'New Bill — ' . $tenant->name)
@section('page-title', 'New Bill')
@section('page-subtitle', 'Record a supplier invoice or company expense')
...
<form method="POST" action="{{ route('tenant.accounting.bills.store', $tenant->slug) }}"
      x-data="billForm()"
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 mb-5">
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="text" name="supplier_vat_number" value="{{ old('supplier_vat_number') }}"
                       class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg">
            </div>
        </div>
        <div class="grid grid-cols-3 gap-4">
            <div>
                ...
                <input type="date" name="bill_date" value="{{ old('bill_date', now()->toDateString()) }}" required
                       class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg">
            </div>
            <div>
...
function billForm() {
    return {
        lines: [{ description: '', category: 'other', amount: '', vat_treatment: 'standard', vat_rate: 5 }],
        addLine() {
