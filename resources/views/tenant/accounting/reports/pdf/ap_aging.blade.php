...
@if($rows->isEmpty())
    <p style="color:#777;font-size:9px">No outstanding payables.</p>
@else
<table>
    <thead>
        <tr>
            <th>Bill #</th><th>Supplier</th><th>Bill Date</th><th>Due Date</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $row['bill']->supplier_name }}</td>
            <td>{{ $row['bill']->bill_date->format('d M Y') }}</td>
            <td>{{ $row['bill']->due_date?->format('d M Y') ?? '—' }}</td>
            <td class="text-right">{{ number_format($row['bill']->total, 2) }}</td>
            <td class="text-right">{{ number_format($row['bill']->amount_paid, 2) }}</td>
