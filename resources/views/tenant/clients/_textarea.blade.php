...
<div>
    ...
    <textarea id="{{ $name }}" name="{{ $name }}" rows="{{ $rows ?? 3 }}"
              class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white resize-none">{{ old($name, $value ?? '') }}</textarea>
</div>
