...
<div x-data="{ tab: new URLSearchParams(window.location.search).get('tab') || 'overview' }">
    ...
    <div class="flex items-center gap-1 bg-white rounded-xl border border-gray-200 p-1 mb-5 overflow-x-auto">
        @foreach([
            ['overview',     'Overview'],
            ['profile',      'Profile'],
        ...
        {{-- Action links in tab bar --}}
        <a href="{{ route('tenant.clients.kyc.pdf', [$tenant->slug, $client->id]) }}"
           class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-indigo-700 border border-indigo-200 rounded-lg hover:bg-indigo-50 transition whitespace-nowrap flex-shrink-0">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
            KYC PDF
        </a>
        <a href="{{ route('tenant.clients.kyc.docx', [$tenant->slug, $client->id]) }}"
           class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-800 border border-blue-300 rounded-lg hover:bg-blue-50 transition whitespace-nowrap flex-shrink-0">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
            KYC Word
        </a>
        <a href="{{ route('tenant.clients.declaration.combined', [$tenant->slug, $client->id]) }}"
           class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-700 border border-blue-200 rounded-lg hover:bg-blue-50 transition whitespace-nowrap flex-shrink-0">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            Declaration
        </a>
        <a href="{{ route('tenant.goaml.create', $tenant->slug) }}?client={{ $client->id }}"
           class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition whitespace-nowrap flex-shrink-0">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    @if(session('screening_log_id') && $client->screening_status === 'match')
                    <a href="{{ route('tenant.screening.log.review', [$tenant->slug, session('screening_log_id')]) }}"
                       class="mt-2 w-full flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition">
                        ...
                        Review Hits — EDD Required
                    </a>
                    @elseif($client->screening_status === 'match')
                    @php
                        $latestMatchLog = \App\Models\ScreeningLog::where('tenant_id', $tenant->id)
                    ...
                    @endphp
                    @if($latestMatchLog)
                    <a href="{{ route('tenant.screening.log.review', [$tenant->slug, $latestMatchLog->id]) }}"
                       class="mt-2 w-full flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                        </svg>
